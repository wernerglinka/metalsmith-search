...
export async function extractSearchableContent(file, filename, options, metalsmith) {
  ...
  try {
    ...
    if (!Buffer.isBuffer(file.contents)) {
      throw new Error(
        `File contents is not a Buffer for ${filename}. Got: ${typeof file.contents}`
      );
    }
    ...
    const pageName = file.pageTitle || file.seo?.title || file.title || 'Untitled Page';
    ...
    if (
      options.indexLevels.includes('section') &&
      Array.isArray(sectionsArray) &&
      sectionsArray.length > 0
    ) {
      // First, ensure all sections have IDs for template rendering
      ensureSectionIDs(sectionsArray);

      const sectionEntries = extractSectionLevelContent(sectionsArray, cleanUrl, options, pageName);
      entries.push(...sectionEntries);
      debug(
        `Added ${sectionEntries.length} component section entries for ${filename} (field: ${options.sectionsField})`
      );
    }
    ...
    if (options.indexLevels.includes('section') && !sectionsArray && file.contents) {
      const traditionalSections = extractTraditionalSections(file, cleanUrl, options, pageName);
      entries.push(...traditionalSections);
      debug(`Added ${traditionalSections.length} traditional section entries for ${filename}`);
    }

    // Component-level indexing not implemented yet - section-level provides sufficient granularity
    // Future: if (options.indexLevels.includes('component')) { ... }
 ...
 */
function extractPageLevelContent(file, url, options, pageName) {
  const content = {};

  // Extract and process basic metadata (handle markdown in frontmatter)
  if (file.title) {
    content.title = processMarkdownField(file.title);
  }
  if (file.description) {
    content.description = processMarkdownField(file.description);
  }
  if (file.excerpt) {
    content.excerpt = processMarkdownField(file.excerpt);
  }
  if (file.tags) {
    content.tags = Array.isArray(file.tags) ? file.tags : [file.tags];
  }
  if (file.date) {
  ...
  for (const field of contentFields) {
    if (file[field]) {
      content[field] = processMarkdownField(file[field]);
    }
  }
 ...
 */
function extractSectionLevelContent(sections, baseUrl, options, pageName) {
  const entries = [];
 ...
 */
function extractTraditionalSections(file, baseUrl, options, pageName) {
  const entries = [];
 ...
 */
function processMarkdownField(fieldValue) {
...
// Future extension: extractComponentLevelContent could be added here for individual component indexing
// Currently, section-level indexing provides sufficient granularity
